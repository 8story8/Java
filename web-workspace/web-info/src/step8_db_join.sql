/*    JOIN SQL : 두 개 이상의 테이블에서 행을 결합하기
 *					  위해 사용하는 SQL
 *    
 *    사례 ) 	사원정보 : 
 ...
 * 
 *     위와 같이 사원수가 늘어날 수록 동일한 부서정보가 
 *     중복되어 저장된다. 
 *     테이블을 분리하여 설계 -> 중복 제거하여 자원을 절약 
 * 
 *     사원테이블 , 부서테이블 
 ...
 * 		부모테이블 ( master table ) : 참조대상테이블을 말한다
 * 		-> 부서테이블 
 *      자식테이블 : 참조하는 테이블을 말한다 ( fk ) 
 *      -> 사원테이블 
 * 		
 ...
 *      제약조건명 references 부모테이블(컬럼명)
 * 
 *      위와 같이 foreign key 제약조건을 주면 참조하는 
 * 		부모 테이블의 컬럼에 존재하지 않는 정보를 insert할때는 
 * 		무결성 제약 조건 에러가 발생한다 
 * 		ex) 부서 테이블의 부서번호정보가 10,20,30이 존재할때
 * 			 사원 테이블의 부서번호 40이 insert 될 경우에는 
 ...
 * 	    두 개 이상의 테이블로부터 자료를 조회하기 위해 사용
 *      일반적으로 pk 와 fk 를 이용해 join 한다 
 * 		사례) 부서테이블의 부서번호와 사원테이블의 부서번호가 상응하는
 * 				컬럼  
 * 
 * 		SELECT 컬럼명...
 * 	    FROM Table1 별칭, Table2 별칭
 * 		WHERE 별칭.컬럼=별칭.컬럼   --> 조인조건 , 조인시 별칭권장 
 */
 ...
 select * from emp;
 select * from dept;
 -- emp 와 dept 테이블을 join sql 로 아래와 같이 정보를 조회하세요
 -- empno , ename , sal , deptno , dname, loc 를 조회( sal 내림차순 ) 
...
create table salgrade(
  ...
  */
  ...
  and e.sal>=s.losal and e.sal<=s.hisal and e.deptno=d.deptno 
...
/* 3. Outer Join 
  ...
  -- Outer join을 이용하여 조회하면 된다 
  select e.empno,e.ename,d.deptno,d.dname
  from emp e, dept d
  where e.deptno(+)=d.deptno
  order by d.deptno asc;
  -- 데이터가 없는 측에 (+)  : Outer join 연산기호를 명시하면 된다 
  ...
  /* 4. Self Join
     ...
     * 
     */
  -- 사원의 사원번호,사원명,매니저, 매니저의사원번호,매니저의 사원명
...
ORDER BY avg_sal DESC
 ...
 --  부서별 사원수 emp_count, 부서별 평균월급 avg_sal 을 조회 
 -- avg_sal 내림차순으로  
SELECT e.deptno,d.dname,e.emp_count,e.avg_sal
FROM(

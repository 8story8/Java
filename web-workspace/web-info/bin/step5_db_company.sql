...
-- 사원 emp 테이블의 사원번호 empno, 사원명 ename, 직종 job, 입사일 hiredate
select empno, ename, job, hiredate from emp;

-- empno 내림차순 정렬하여 조회
select empno, ename, job, hiredate from emp order by empno desc;

-- job이 ANALYST인 사원의 사원명 ename과 월급 sal을 조회
select ename, sal from emp where job = 'ANALYST';

-- 모든 사원의 사원명, 직종, 월급, 연봉(월급 * 12)을 월급 내림차순 정렬
select ename, job, sal, sal*12 as asal from emp order by sal desc;
...
-- 월급 sal 1000이상 2000이하 사원의 ename, job, sal을 sal 내림차순으로 조회
...
-- clob : 최대 4GB를 지원
-- ref) varchar2 : 최대 4000B 지원
create table clob_test(
	id varchar2(100) primary key,
	content clob not null
);

insert into clob_test values('Bourbon', '도레미파솔라시도시라솔파미레도레미파솔라도시라솔파미레도');

select * from clob_test;

-- default 제약 조건(기본값 명시)
create table default_test(
	id varchar2(100) primary key,
	hit number default 0
);

insert into default_test(id) values('Vermouth');
...
insert into board_test(b_no, content) values(1, '칠무해');
...
insert into board_test(b_no, content) values(2, '사황');

-- update를 이용해 hit가 null -> 0으로 모두 수정 

 ...
 *   		model
 *   				MemberService	<--import    Log4J lib
 *   				ProductService
 *   
 *   log info level 로  java-kosta 이하에 project.log 에 로깅처리
 *   
 *   maven  pom.xml 
 *   log4j.xml 
 ...
 *   --> 요구사항(공통관심사항)에 대한 처리 작업을 
 *   	   효과적으로 구현하고 유지보수할 수 있는 방안 
 *   	   --> AOP(Aspect Oriented Programming)	
 *   
 *   방안2 AOP 를 적용한다 
 *   Aspect Oriented Programming은 
 *   시스템을 핵심관심사항과 공통(횡단)관심사항으로
 *   구분하여 공통관심사항에 대한 작업을
 *   별도의 모듈에서 처리하도록 한다 	
 *   -> 기존 소스코드를 수정하지 않고 
 *   	 별도의 모듈에서 한번 구현하고 유지보수하므로
 *       생산성 및 유지보수성을 향상시킬 수 있다 
 *       
 *   how? 0. maven : aop library 추가 
 *   		1. 공통관심사항을 정의하는 클래스 구현 

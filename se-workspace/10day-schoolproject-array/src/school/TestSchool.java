...
public class TestSchool 
{
	public static void main(String[] args) 
	{
		...
		SchoolService service = new SchoolService(100);							// SchoolService의 멤버 변수 personList의 배열(length 100) 생성
		...
		System.out.println("**********step3. 동일 tel 등록시 추가 불가**********");
		service.addPerson(new Employee("019", "만송이", "강남", "영업"));			// 019번호가 존재하여 추가 불가!
		...
		service.updatePerson(new Teacher("010", "백만송이", "영등포", "수학"));		// 존재하지 않는 010 객체를 전달하면
		up = service.findPersonByTel("010");									// 010번호를 가진 구성원이 없으므로 수정 불가!
		System.out.println(up);													// null

		System.out.println("**********step5. 구성원 삭제 전**********");
		service.printAll(); 													// 4명 출력
		service.deletePersonByTel("017");

		System.out.println("**********step5. 구성원 삭제 후**********");
